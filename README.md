...
- **13大策略 x 6大维度**：技术+基本面+消息+政策+情绪+资金+业绩增长，全方位覆盖
...
- **策略相关性管理**：高相关策略（资金流/新闻/情绪）自动折扣，避免信号重复计数
- **L2 动态权重**：基于个股波动率自动调整策略权重，高波动股偏重技术面、低波动股偏重基本面
...
|   |   |-- ensemble.py                 # 组合策略（13策略加权+L2动态权重+相关性管理）
