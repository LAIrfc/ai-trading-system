...
- ✅ **双引擎调度** - 均值回归引擎 + 趋势引擎并行，智能融合
  - **均值回归引擎**：12策略加权投票（PB/PE/PEPB/DUAL/BOLL/RSI/KDJ/MACD/MA）
  - **趋势引擎**：Trend_Composite（ADX+均线排列+波动率调整动量）
  - **软过滤机制**：趋势得分调整均值回归权重，避免硬剔除
  - **市场状态感知**：指数MA20/MA60斜率判断趋势市/震荡市
- ❌ **L1/L2: 动态权重调整** - 经验证后永久禁用（固定权重表现更优）

**双引擎调度特性**（v5.0新增）：
- **均值回归引擎**：
  - 策略权重：PB(2.0), BOLL(1.95), RSI(1.82), PE(1.68), PEPB(1.61), KDJ(1.5), DUAL(1.39), MACD(1.13), MA(0.88)
  - 基于836只股票回测优化（composite方法）
  - 输出：超跌反弹型推荐
- **趋势引擎**：
  - 核心策略：Trend_Composite（复合趋势因子）
  - 得分公式：0.7×trend_score + 0.3×momentum_score
  - 输出：趋势跟随型推荐
- **软过滤**：adjusted_score = mr_score × (0.5 + 0.5×trend_weight)
- **市场状态**：
  - 趋势市（MA20/MA60斜率>3%）：超跌榜10只 + 趋势榜10只
  - 震荡市（MA20/MA60斜率≤3%）：超跌榜15只 + 趋势榜5只

**推荐机制**：
- **双榜单输出**：超跌反弹榜 + 趋势跟随榜，风格多样化
- **智能去重**：趋势榜自动排除超跌榜中的股票
- **动态比例**：根据市场状态自动调整双榜单数量
- **专题板块**：自动追加专题板块分析（如核电+算电协同）
...
| 模块 | 说明 |
|------|------|
| **每日推荐** | 12策略从808只股票池筛选TOP 20，自动生成增量跟踪文档 📊 |
| **健康上涨组** | 从全市场筛选趋势跟随型标的（价格站稳MA20 + 健康上涨），与超跌反弹型互补 📈 |
| **专题板块推荐** | 自动追加专题板块分析（如核电+算电协同），无需手动操作 🔋 |
| **持仓跟踪** | 记录买入/卖出操作，追踪盈亏，每日自动更新 💼 |
| **策略选股** | 12策略 Full Ensemble（技术6+基本面3+消息面+资金面+情绪面）从综合大池筛选，加权评分排序 |
...
12个子策略加权投票，综合技术面、基本面、消息面、资金面和情绪面信号：
