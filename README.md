...
- **共振结构判断**（2026-04 新增）：5组独立信号（趋势/超跌反转/估值/基本面/事件），多组共振降阈值，单组信号升阈值（默认关闭，监控中，待策略活跃度提升后启用）
- **三级推荐+空仓能力**（2026-04 新增）：STRONG/NORMAL/WEAK/EMPTY四级，纯信号指标（buy_ratio/avg_buy_count/positive_ratio），极弱市场自动空仓
- **进场区域标签**（2026-04 新增）：6级位置标签（核心买点→不建议新开仓），需3+危险条件才标⛔，区分高位与高位衰竭
- **NEWS过热打折**（2026-04 新增）：20日涨幅>25%且新闻密集时，利好置信度自动打折
- **技术策略弱信号**（2026-04 新增）：MACD/RSI/BOLL/MA增加趋势延续弱BUY/SELL（回测验证：MACD +9.55%/68%胜率，BOLL +10.01%/83%胜率），KDJ弱信号因胜率33%已移除
- **PE/PB多源降级**（2026-04 新增）：baostock→百度股市通→全市场spot查表，历史PE/PB数据自动缓存7天，解决baostock网络故障时PE策略失活问题
- **大市值分级惩罚**（2026-04 修订）：500-1000亿×0.75（龙头恢复×0.90），1000-2000亿×0.60，>2000亿×0.50（替代原>1000亿硬出局）
- **策略相关性管理**：高相关策略组内自动折扣（RSI/KDJ/BOLL、MA/MACD、PE/PB/PEPB），避免信号重复计数
- **L2 弱动态权重**：个股波动率调整 + 市场状态微调（默认关闭，监控中，待策略活跃度提升后启用）
- **业绩增长策略**：优先使用季报实际数据，辅以预告数据，并支持行业景气度外推（同行已出季报推测未出标的）
- **NEWS实时分析**：每次运行都重新拉取最新新闻+LLM分析，保证数据新鲜度。32线程并发调用DeepSeek API
- **3年实证回测**：863只股票 x 145个调仓周期，v6.4 累计+42.51%，最大回撤-43.16%，跑赢沪深300 约31%
- **本地缓存体系**：K线(parquet)+PE/PB+基本面+新闻(JSON)四级缓存，增量更新
- **AI驱动分析**：新闻情感+政策事件（关键词40%+LLM语义60%），DeepSeek API
- **东财妙想集成**：实时行情、选股筛选、新闻搜索、自选股同步
- **资金追踪**：龙虎榜机构席位+大宗交易折价分析
- **板块专项分析**：支持按股票代码或板块名进行定向多策略分析
---
...
```
全市场股票池（~1100只：沪深300 + 中证500 + 科创50 + 创业板300 + 创业板活跃补充）
    |
[L0: PolicyEvent 大盘过滤]
...
|   同时出现在超跌榜和趋势榜的标记为"双优"（估值+动能双重确认）
    |
[市场机会评估: STRONG/NORMAL/WEAK/EMPTY]
|   极弱市场空仓，弱市场只输出观察池
    |
最终推荐（按级别输出 TOP 8~20 + 进场区域标签 + 操作建议）
```
...
| EARNINGS_GROWTH | 1.20 | 季报实际盈利增长（优先）→ 预告预测 → 行业景气度外推 |
...
**L2 动态权重**：基础权重会根据个股波动率自动调整 — 高波动股偏重技术面策略（RSI/BOLL/KDJ），低波动股偏重基本面策略（PE/PB/EARNINGS_GROWTH）。

**相关性管理**：高相关策略对（如 NEWS 与 MONEY_FLOW）的信号会自动折扣，避免同一类信号被重复计数。

**选股逻辑**：每只股票跑13个策略 → L2动态权重调整 → 相关性折扣 → 加权求和得到总分 → 按总分排序 → 取 TOP 15。
...
**模型A — 长周期十倍股（3-5年）** `src/strategies/tenbagger_model.py`
- 评分公式：`十倍评分 = 利润质量×30% + 增长持续性×25% + 竞争壁垒×15% + 长期赛道×15% + 估值安全×15%`
- 重利润质量：扣非增速、经营现金流、毛利率稳定性、ROE趋势、一次性收益占比
- 核心因子：PEG < 1、利润质量评级 ≥ B
- 市值区间：30-200亿（>200亿打折，>500亿淘汰）
- 长期赛道：半导体、创新药、机器人、军工、储能、自动驾驶、高端制造等
- 逻辑：穿越周期、长期产业趋势、利润释放

**模型B — 短周期翻倍股（3-6个月）**
- 重资金与催化：行业热度、资金强度、催化密度、预期差、筹码集中度
- 评分公式：`翻倍概率 = 行业热度 × 催化密度 × 资金强度 × 预期差 × 筹码集中度`
- 市值区间：30-500亿
- 热点赛道：AI算力、机器人、半导体、并购重组、低空经济、储能、光伏
- 关键信号：均线多头、主力净流入、融资余额上升、股东户数下降

**黄金交叉**：同时具备长期十倍逻辑 + 短期资金启动的个股，被标记为"黄金交叉"，风险收益比最优。
...
| PROFIT_QUALITY | 1.40 | 利润质量5维评分：扣非增速、经营现金流/净利润比、毛利率稳定性、ROE趋势、一次性收益占比 |

PE/PB 使用3年滚动窗口（约528个交易日），每日计算当前值在历史中的百分位排名。低分位表示当前估值处于历史低位，有均值回归的空间。

利润质量因子区分"真实业绩拐点"和"财技利润"：经营现金流/净利润 > 0.8 且一次性收益占比 < 15% 表明利润真实可持续。

### 业绩增长策略（1个）

| 策略 | 基础权重 | 核心逻辑 |
|------|---------|---------|
| EARNINGS_GROWTH | 1.20 | 三级数据源优先级：季报实际盈利 > 预告预测 > 行业景气度外推 |

EARNINGS_GROWTH 策略的数据获取优先级：
1. **季报实际数据**（`stock_yjbb_em`）：已披露的最新季度报告，盈利增速直接计算
2. **业绩预告**（`stock_yjyg_em`）：取预告上下限均值估算增速
3. **行业景气度外推**：当本公司数据均无时，利用同板块已出季报的同行业绩推测景气度（正增长同行占比 > 60% 视为景气）

### 消息/情绪/资金策略（3个）

| 策略 | 基础权重 | 数据源 | 核心逻辑 |
|------|---------|--------|---------|
| NEWS | 0.32 | 东方财富/新浪/同花顺 | 近24h新闻情感分析（关键词匹配40% + LLM语义理解60%），正面情绪加分。磁盘缓存2小时TTL |
| SENTIMENT | 0.32 | 涨跌停/成交量/融资融券 | 恐慌贪婪指数（多指标Z-score合成），极度恐慌时逆向加分 |
| MONEY_FLOW | 0.30 | 龙虎榜/大宗交易 | 机构席位净买入 + 大宗交易折价率分析，跟踪聪明钱 |

### 行业趋势策略（1个，2026-04 新增）

| 策略 | 基础权重 | 核心逻辑 |
|------|---------|---------|
| INDUSTRY_TREND | 0.35 | LLM分析行业景气度、政策催化、产业链上下游传导，判断行业拐点 |

### 政策过滤（L0层）

| 策略 | 层级 | 核心逻辑 |
|------|------|---------|
| POLICY | L0过滤 | 极度利空（降准降息重大转向、IPO暂停等）时全市场暂停选股 |

### 十倍股模型（独立评分，2026-04 新增）

| 维度 | 权重 | 评估内容 |
|------|------|---------|
| 利润质量 | 30% | 扣非增速、经营现金流覆盖率、毛利率稳定性、ROE变化、一次性收益占比 |
| 增长持续性 | 25% | 营收/利润连续增长、PEG、基本面综合评分 |
| 竞争壁垒 | 15% | 品牌/技术溢价(PB)、市值档位、行业地位、策略信号一致性 |
| 长期赛道 | 15% | 半导体/创新药/机器人/军工/储能/自动驾驶/高端制造等长期成长赛道 |
| 估值安全 | 15% | PEG、PE/PB历史分位数、估值合理性 |

### 翻倍股模型（独立评分，2026-04 新增）

| 维度 | 权重 | 评估内容 |
|------|------|---------|
| 行业热度 | 25% | 是否属于当前热点赛道（AI算力/机器人/半导体/并购重组/低空经济/储能/光伏） |
| 资金强度 | 25% | 主力净流入、融资余额趋势、龙虎榜活跃度 |
| 催化密度 | 20% | 未来3个月重大事件（新订单/新产能/重组/大客户导入/行业政策/业绩预告） |
| 预期差 | 15% | 一致预期与实际的偏离度，市场是否存在低估 |
| 筹码集中度 | 15% | 股东户数趋势、流通市值、龙头辨识度 |

### L2 动态权重与相关性管理

**动态权重**：基于个股波动率比率（个股20日波动率 / 市场中位波动率）自动调整：
- 高波动股（比率 > 1.5）：技术面策略权重 x 1.3，基本面策略权重 x 0.8
- 低波动股（比率 < 0.7）：技术面策略权重 x 0.8，基本面策略权重 x 1.2

**相关性折扣**：预定义的高相关策略对（如 NEWS-MONEY_FLOW、NEWS-SENTIMENT）的分数取最大值后折扣30%，防止同类信号被多次计入。
...
# 按板块名搜索分析
python3 tools/analysis/sector_analyze.py --sector "算力" --top 10
```
...
```bash
# 对比策略变更前后的表现差异
python3 tools/analysis/ab_test_changes.py
```
...
| 文件 | 说明 | 数量 | 来源 |
|------|------|------|------|
| `mydate/stock_pool_all.json` | 综合池（每日选股唯一数据源） | ~1100只 | 沪深300+中证500+科创50+创业板指(399006)+创业板200(399019)+创业板活跃补充(成交额TOP100) |
| `mydate/etf_pool.json` | ETF池（独立，不参与每日推荐） | 57只 | 宽基/科技/消费/金融/周期/医药/地产/跨境 |
...
|   |   |-- earnings_growth.py          # EARNINGS_GROWTH 业绩增长（季报+预告+行业外推）
|   |   |-- news_sentiment.py           # NEWS 新闻情感分析（含磁盘缓存2h TTL）
...
|   |   |-- fundamental/               # 基本面数据
|   |   |   `-- profit_quality.py       # 利润质量因子（5维评分）
|   |   |-- ai_analyst.py              # LLM 调用封装（DeepSeek API，timeout=15s）
...
| [docs/strategy/STRATEGY_DETAIL.md](docs/strategy/STRATEGY_DETAIL.md) | 策略信号逻辑、参数、置信度映射 |
| [docs/strategy/STRATEGY_QUICKSTART.md](docs/strategy/STRATEGY_QUICKSTART.md) | 5分钟上手策略开发 |
| [docs/strategy/STRATEGY_LIST.md](docs/strategy/STRATEGY_LIST.md) | 完整策略清单 |
...
| [docs/data/FUNDAMENTAL_DATA.md](docs/data/FUNDAMENTAL_DATA.md) | 基本面数据（含利润质量因子）|
