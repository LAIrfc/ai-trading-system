    ...
    def analyze(self, df: pd.DataFrame) -> StrategySignal:
        ...
        if prev_k <= prev_d and cur_k > cur_d:
            ...
            if cur_k < 30:
                ...
                position = (self._LOW_CROSS_POS_MIN +
                ...
                position = (self._MID_CROSS_POS_MIN +
        ...
        if cur_j < 0:
            if cur_j > prev_j:
                ...
                confidence = (self._J_BASE_CONF +
                              j_factor * (self._J_MAX_CONF -
                                          self._J_BASE_CONF))
                position = (self._J_BUY_POS_MIN +
                ...
                return StrategySignal(
                    action='BUY', confidence=round(confidence, 2),
                    position=round(position, 2),
                    reason=f'J值超卖拐头 ({prev_j:.1f}→{cur_j:.1f}){vol_desc}',
                    indicators=indicators,
                )
            else:
                return StrategySignal(
        ...
        if cur_j > 100:
            if cur_j < prev_j:
                ...
                return StrategySignal(
                    action='SELL', confidence=round(confidence, 2),
                    position=round(position, 2),
                    reason=f'J值超买拐头 ({prev_j:.1f}→{cur_j:.1f}){vol_desc}',
                    indicators=indicators,
                )
            else:
                return StrategySignal(
