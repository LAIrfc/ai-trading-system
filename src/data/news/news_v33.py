...
def _llm_score_stock_news(
    ...
    try:
        ...
        if event_driven and isinstance(event_driven, dict):
            has_catalyst = bool(event_driven.get("has_catalyst", False))
            catalyst_score = float(event_driven.get("catalyst_score", 0))
            catalyst_score = max(0.0, min(1.0, catalyst_score))
            if has_catalyst and catalyst_score > 0.3:
                boost = catalyst_score * 0.4
...
def get_news_sentiment_v33(
    ...
    Returns
    -------
    (S_news, N, mean_source_weight, event_driven) 或 None
        S_news 为情感分 [-1, 1]；N 为同向篇数；mean_source_weight 用于置信度；
        event_driven 为事件催化剂信息 dict 或 None。
    """
    try:
        ...
        if llm_result is not None:
            ...
            event_driven = llm_result.get("event_driven")
            logger.info(
                "[%s] 新闻情感 [关键词%.2f × LLM%.2f → 融合%.2f] %s 主要事件:%s 催化剂:%s",
                symbol, kw_score, llm_score, final_score,
                llm_result.get("direction", ""),
                ",".join(llm_result.get("key_events", [])),
                event_driven.get("catalyst_type", "无") if event_driven else "无",
            )
            return (final_score, max(N, 1), mean_weight, event_driven)

        # LLM 不可用，退化为纯关键词
        if N == 0:
            return None
        logger.debug("[%s] 新闻情感：LLM 不可用，使用纯关键词分数 %.2f (N=%d)", symbol, kw_score, N)
        return (kw_score, N, mean_weight, None)
