...
Uses a hardcoded holiday set for 2025–2026 (official exchange schedules plus
approximate 2026 dates where not yet published). Outside those years, only
weekends are treated as non-trading days (no holiday knowledge).
"""
...
def _build_cn_holidays() -> frozenset[date]:
    ...
    # --- 2026 (approximate; refine when exchange notice is published) ---
    _add_range(h, _d(2026, 1, 1), _d(2026, 1, 1))  # 元旦
    # 春节：农历约在 2 月中下旬，此处按约 9 天休市占位
    _add_range(h, _d(2026, 2, 15), _d(2026, 2, 23))
    _add_range(h, _d(2026, 4, 4), _d(2026, 4, 6))  # 清明
    _add_range(h, _d(2026, 5, 1), _d(2026, 5, 5))  # 劳动节
    _add_range(h, _d(2026, 6, 19), _d(2026, 6, 21))  # 端午节（农历五月初一附近）
    _add_range(h, _d(2026, 9, 25), _d(2026, 9, 27))  # 中秋节（近似）
    _add_range(h, _d(2026, 10, 1), _d(2026, 10, 7))  # 国庆节
